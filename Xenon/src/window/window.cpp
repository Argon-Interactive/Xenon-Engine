...
Core::Window::Window(uint32_t width, uint32_t height, std::string title)
	:m_isVSync(true), m_isBorderless(false), m_ID(nullptr), m_title(title), m_monitor(nullptr), m_icon{0, 0}, m_winPosX(0), m_winPosY(0), m_winSizeW(width), m_winSizeH(height) {} //NOLINT
Core::Window::~Window()
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set fullscreen."); return; }
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set borderless."); return; }
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set size."); return; }
	glfwSetWindowSize(m_ID, width, height);
}

void Core::Window::setWindowSize(glm::uvec2 dims)
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set size."); return; }
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set position."); return; }
	glfwSetWindowPos(m_ID, static_cast<int>(x), static_cast<int>(y));
}

void Core::Window::setWindowPos(glm::uvec2 pos)
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set position."); return; }
...
{ 
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set size limits."); return; }
	glfwSetWindowSizeLimits(m_ID, static_cast<int>(minW), static_cast<int>(minH), static_cast<int>(maxW), static_cast<int>(maxH));
}

void Core::Window::setSizeLimits(glm::uvec2 minDims, glm::uvec2 maxDims)
{ 
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set size limits."); return; }
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set a title."); return; }
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before setting a frame end point."); return; }
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set an icon."); return; }
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set it's maximization."); return; }
	if (maximize)
...
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to set it's resizing."); return; }
...
glm::uvec2 Core::Window::getWindowSize()
{
	int w, h;
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to get window size."); return {}; }
	glfwGetWindowSize(m_ID, &w, &h);
	return { static_cast<uint32_t>(w), static_cast<uint32_t>(h) };
}

glm::ivec2 Core::Window::getWindowPos()
{
	int x, y;
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to get window position."); return {}; }
	glfwGetWindowSize(m_ID, &x, &y);
	return { x, y };
}

glm::uvec2 Core::Window::getScreenResolution()
{
	if (m_ID == nullptr)
	{ XN_LOG_ERR("Window hasn't yet been created. First create a window before trying to get screen resolution."); return {}; }
	const GLFWvidmode* mode = glfwGetVideoMode(m_monitor);
